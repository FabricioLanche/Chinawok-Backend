...
def lambda_handler(event, context):
    ...
    if not rol:
        try:
            ...
            return {'statusCode': 500, 'body': json.dumps({'error': f"Error al buscar empleado: {str(e)}"})}
    ...
    if rol == 'cocinero':
        ...
        return {'statusCode': 400, 'body': json.dumps({'error': f"Rol '{rol}' no válido. Debe ser cocinero, despachador o repartidor"})}
    ...
    try:
        ...
        return {'statusCode': 500, 'body': json.dumps({'error': f"Error al consultar reseñas: {str(e)}"})}
