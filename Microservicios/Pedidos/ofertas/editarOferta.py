...
def handler(event, context):
    ...
    try:
        ...
        campos_permitidos = ['producto_nombre', 'combo_id', 'fecha_inicio', 'fecha_limite', 'porcentaje_descuento']
        ...
        # Validar porcentaje_descuento si se está actualizando
        if 'porcentaje_descuento' in update_data:
            porcentaje = update_data['porcentaje_descuento']
        ...
        return {
            'statusCode': 200,
            'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
            'body': json.dumps({'message': 'Oferta actualizada exitosamente', 'data': response['Attributes']}, default=str)
        }
